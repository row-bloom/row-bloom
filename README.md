...
[<img src="https://github-ads.s3.eu-central-1.amazonaws.com/row-bloom.jpg?t=1" width="419px" />](https://spatie.be/github-ad-click/row-bloom)
...
```bash
php artisan vendor:publish --tag="row-bloom-migrations"
php artisan migrate
```
...
```bash
php artisan vendor:publish --tag="row-bloom-config"
```
...
```bash
php artisan vendor:publish --tag="row-bloom-views"
```

## Usage

```php
$rowBloom = new ElaborateCode\RowBloom();
echo $rowBloom->echoPhrase('Hello, ElaborateCode!');
```
