...
it('factorize')
    ->expect(fn () => app()->make(RendererFactory::class)->make(Renderer::Html))
    ->toBeInstanceOf(HtmlRenderer::class);
...
it('renders and get (basic)')
    ...
    ->expect(function ($template, $css, $options) {
        return app()->make(RendererFactory::class)->make(Renderer::Html)->render($template, $css, $options)->get();
    })
    // ? more assertions
    ->toBeString();
